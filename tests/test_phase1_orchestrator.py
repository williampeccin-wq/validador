...
def test_phase1_gate1_inventory():
    ...
    # Ajuste os caminhos para seus fixtures reais
    collect_document(case_id, "tests/fixtures/andersonsantos.pdf", document_type="proposta_daycoval")
    st1 = case_status(case_id)
    assert st1.is_complete is False
    assert "cnh" in st1.missing

    collect_document(case_id, "tests/fixtures/CNH DIGITAL.pdf", document_type="cnh")
