...
def _neutralize_income_to_ok(phase1_case: Path) -> None:
    """
    Ensures income checks do NOT produce MISSING/WARN so we can isolate ATPV RENAVAM policy behavior.
    """
...
def test_renavam_required_if_supported_emits_missing_and_degrades_overall(tmp_path: Path) -> None:
    ...
    # Neutralize income checks
    _neutralize_income_to_ok(phase1_case)

    # Correlate vehicle doc present -> supports requiring RENAVAM
    _write_phase1_doc(phase1_case, "crlv_e", {"placa": "ASY6E68", "chassi": "9BD118181B1126184"})

    # ATPV without RENAVAM -> should emit MISSING on required_if_supported
    _write_phase1_doc(
        phase1_case,
        "atpv",
        {
    ...
    # Required_if_supported is a true MISSING -> overall_status should be MISSING
    assert report.get("overall_status") == "MISSING"
    assert (report.get("summary") or {}).get("overall_status") == "MISSING"

    # Policy A still OK
    assert _find_check(report, "vehicle.atpv.comprador.matches_proposta")["status"] == "OK"


def test_renavam_required_if_supported_ok_when_valid_renavam_present(tmp_path: Path) -> None:
    r = _mk_case_roots(tmp_path)
    case_id = str(r["case_id"])
    phase1_case = r["phase1_case_root"]  # type: ignore[assignment]
    ...
    # Correlate vehicle doc present
    _write_phase1_doc(phase1_case, "crlv_e", {"placa": "ASY6E68", "chassi": "9BD118181B1126184"})

    # RENAVAM 11-digit with valid DV (example: base 1234567890 + dv 0)
    _write_phase1_doc(
        phase1_case,
        "atpv",
        {
    ...
    # With everything else neutralized, overall should remain OK
